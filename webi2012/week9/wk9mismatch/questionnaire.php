<?php
  ...
  if (mysqli_num_rows($data) == 0) {
    ...
    $query = "SELECT topic_id FROM wk9mismatch_topic ORDER BY category_id, topic_id";
    $data = mysqli_query($dbc, $query);
    $topicIDs = array();
    ...
    foreach ($topicIDs as $topic_id) {
      $query = "INSERT INTO wk9mismatch_response (user_id, topic_id) VALUES ('" . $_SESSION['user_id']. "', '$topic_id')";
  ...
  if (isset($_POST['submit'])) {
    ...
    foreach ($_POST as $response_id => $response) {
      $query = "UPDATE wk9mismatch_response SET response = '$response' WHERE response_id = '$response_id'";
      mysqli_query($dbc, $query);
    }
  ...
  $query = "SELECT mr.response_id, mr.topic_id, mr.response, mt.name AS topic_name, mc.name AS category_name " .
    ...
    "INNER JOIN wk9mismatch_category AS mc USING (category_id) " .
