<?php
  ...
  $query = "SELECT * FROM wk9mismatch_response WHERE user_id = '" . $_SESSION['user_id'] . "'";
  $data = mysqli_query($dbc, $query);
  
  $user_cat_lover = 0;
  $current_user_cat_lover = 0;
  $mismatch_user_cat_lover = 0;
  ...
  if (mysqli_num_rows($data) != 0) {
    ...
    $query = "SELECT mr.response_id, mr.topic_id, mr.response, mt.name AS topic_name " .
      "FROM wk9mismatch_response AS mr " .
      "INNER JOIN wk9mismatch_topic AS mt USING (topic_id) " .
      "WHERE mr.user_id = '" . $_SESSION['user_id'] . "'";
    $data = mysqli_query($dbc, $query);
    $user_responses = array();
    while ($row = mysqli_fetch_array($data)) {
        if($row['topic_name']=='Cats'){
           $user_cat_lover = $row['response'];
        }
    ...
    $query = "SELECT user_id FROM wk9mismatch_user WHERE user_id != '" . $_SESSION['user_id'] . "'";
    $data = mysqli_query($dbc, $query);
    while ($row = mysqli_fetch_array($data)) {
      // Grab the response data for the user (a potential mismatch)
       $query2 = "SELECT mr.response_id, mr.topic_id, mr.response, mt.name AS topic_name " .
      "FROM wk9mismatch_response AS mr " .
      "INNER JOIN wk9mismatch_topic AS mt USING (topic_id) " .
      "WHERE mr.user_id = '" . $row['user_id'] . "'";
      $data2 = mysqli_query($dbc, $query2);
      $mismatch_responses = array();
      while ($row2 = mysqli_fetch_array($data2)) {
        if($row2['topic_name']=='Cats'){
            $current_user_cat_lover = $row2['response'];
             $mismatch_user_cat_lover = $current_user_cat_lover;
        }
    ...
    if ($mismatch_user_id != -1) {
      $query = "SELECT username, first_name, last_name, city, state, picture FROM wk9mismatch_user WHERE user_id = '$mismatch_user_id'";
      $data = mysqli_query($dbc, $query);
      if (mysqli_num_rows($data) == 1) {
        // The user row for the mismatch was found, so display the user data
        $row = mysqli_fetch_array($data);
        
         if ($mismatch_user_cat_lover != $user_cat_lover) 
        {  
            echo "<p>Cat Mismatch!</p>";
        }
