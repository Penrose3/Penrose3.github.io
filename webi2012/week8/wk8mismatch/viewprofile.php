<?php
  ...
  if (!isset($_GET['user_id'])) {
    $query = "SELECT username, first_name, last_name, gender, birthdate, city, state, picture, user_id FROM wk8mismatch_user WHERE user_id = '" . $_SESSION['user_id'] . "'";
  }
  else {
    $query = "SELECT username, first_name, last_name, gender, birthdate, city, state, picture, user_id FROM wk8mismatch_user WHERE user_id = '" . $_GET['user_id'] . "'";
  }
  ...
  if (mysqli_num_rows($data) == 1) {
    ...
    if ($_SESSION['usertype'] == 1) {
          echo '<p>Would you like to <a href="editprofile.php?user_id=' . $row['user_id'] . '">edit this profile</a>?</p>';
    }
    else if (!isset($_GET['user_id']) || ($_SESSION['user_id'] == $_GET['user_id'])) {
