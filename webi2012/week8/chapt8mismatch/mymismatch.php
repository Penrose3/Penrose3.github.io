<?php
  ...
  if (mysqli_num_rows($data) != 0) {
    ...
    $query = "SELECT user_id FROM chapt8mismatch_user WHERE user_id != '" . $_SESSION['user_id'] . "'";
    $data = mysqli_query($dbc, $query);
    while ($row = mysqli_fetch_array($data)) {
      // Grab the response data for the user (a potential mismatch)
      $query2 = "SELECT response_id, topic_id, response FROM chapt8mismatch_response WHERE user_id = '" . $row['user_id'] . "'";
