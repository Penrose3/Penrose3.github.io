<?php
  ...
  $query = "SELECT * FROM chapt8mismatch_response WHERE user_id = '" . $_SESSION['user_id'] . "'";
  $data = mysqli_query($dbc, $query);
  if (mysqli_num_rows($data) == 0) {
    // First grab the list of topic IDs from the topic table
    $query = "SELECT topic_id FROM chapt8mismatch_topic ORDER BY category_id, topic_id";
    $data = mysqli_query($dbc, $query);
    $topicIDs = array();
    ...
    foreach ($topicIDs as $topic_id) {
      $query = "INSERT INTO chapt8mismatch_response (user_id, topic_id) VALUES ('" . $_SESSION['user_id']. "', '$topic_id')";
  ...
  if (isset($_POST['submit'])) {
    ...
    foreach ($_POST as $response_id => $response) {
      $query = "UPDATE chapt8mismatch_response SET response = '$response' WHERE response_id = '$response_id'";
      mysqli_query($dbc, $query);
    }
  ...
  $query = "SELECT mr.response_id, mr.topic_id, mr.response, mt.name AS topic_name, mc.name AS category_name " .
    "FROM chapt8mismatch_response AS mr " .
    "INNER JOIN chapt8mismatch_topic AS mt USING (topic_id) " .
    "INNER JOIN chapt8mismatch_category AS mc USING (category_id) " .
